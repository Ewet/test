...
<template>
    <Form inline  class="toolBarForm">
      ...
      <FormItem  label='运营商名称'>
        <OperatorSelectInput v-model="filterObj.operatorId" @itemSelect="val=>setOperator(val)"></OperatorSelectInput>
      </FormItem>
      <FormItem  label='商户名称'>
...
import { getExcel } from 'views/report/DailyReceipt/model';
...
export default {
  ...
  methods: {
    setOperator (row) {
      this.filterObj.operatorName = row.name;
    },
    newGuid () {
      let guid = '';
      for (let i = 1; i <= 32; i++) {
    ...
    showExport (data) {
      const tmp = [];
      if (this.filterObj.operatorId) {
        tmp.push({ text: `运营商:${this.filterObj.operatorName}`, value: this.filterObj.operatorId, name: 'operator_id' });
      }
      if (this.filterObj.merchantName) {
        tmp.push({ text: `商户:${this.filterObj.merchantName}`, value: this.filterObj.merchantName, name: 'name' });
      }
    ...
    handleSubmit () {
      ...
      this.filterData(Object.assign({
        operator_id: this.filterObj.operatorId,
